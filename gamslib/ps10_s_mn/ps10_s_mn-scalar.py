#  NLP written by GAMS Convert at 12/13/18 11:31:04
#  
#  Equation counts
#      Total        E        G        L        N        X        C        B
#         41       11       30        0        0        0        0        0
...
m.obj = Objective(expr=   0.0483885699475918*m.x11 + 0.0111072443765899*m.x12 + 0.0410200007574789*m.x13
                        + 0.150028368059464*m.x14 + 0.180270475757508*m.x15 + 0.00364568394136257*m.x16
                        + 0.0780046969156387*m.x17 + 0.227090054008436*m.x18 + 0.104605623849374*m.x19
                        + 0.155839282386557*m.x20 - 0.0483885699475918*m.x21 - 0.0111072443765899*m.x22
                        - 0.0410200007574789*m.x23 - 0.150028368059464*m.x24 - 0.180270475757508*m.x25
                        - 0.00364568394136257*m.x26 - 0.0780046969156387*m.x27 - 0.227090054008436*m.x28
                        - 0.104605623849374*m.x29 - 0.155839282386557*m.x30, sense=maximize)
...
m.c32 = Constraint(expr=   m.x1 - m.x2 >= 0)

m.c33 = Constraint(expr=   m.x2 - m.x3 >= 0)

m.c34 = Constraint(expr=   m.x3 - m.x4 >= 0)

m.c35 = Constraint(expr=   m.x4 - m.x5 >= 0)

m.c36 = Constraint(expr=   m.x5 - m.x6 >= 0)

m.c37 = Constraint(expr=   m.x6 - m.x7 >= 0)

m.c38 = Constraint(expr=   m.x7 - m.x8 >= 0)

m.c39 = Constraint(expr=   m.x8 - m.x9 >= 0)

m.c40 = Constraint(expr=   m.x9 - m.x10 >= 0)

m.c41 = Constraint(expr=   m.x10 >= 0)
