...
m.x7 = Var(within=Reals,bounds=(0.1,None),initialize=15)
m.x8 = Var(within=Reals,bounds=(0.1,None),initialize=15)
m.x9 = Var(within=Reals,bounds=(0.1,None),initialize=15)

m.obj = Objective(expr=log(0.398942448887604*(m.x1/m.x7*exp(-0.5*((95 - m.x4)/m.x7)**2) + m.x2/m.x8*exp(-0.5*((95 - m.x5
                       )/m.x8)**2) + m.x3/m.x9*exp(-0.5*((95 - m.x6)/m.x9)**2))) + log(0.398942448887604*(m.x1/m.x7*exp(
                       -0.5*((105 - m.x4)/m.x7)**2) + m.x2/m.x8*exp(-0.5*((105 - m.x5)/m.x8)**2) + m.x3/m.x9*exp(-0.5*((
                       105 - m.x6)/m.x9)**2))) + 4*log(0.398942448887604*(m.x1/m.x7*exp(-0.5*((110 - m.x4)/m.x7)**2) + 
                       m.x2/m.x8*exp(-0.5*((110 - m.x5)/m.x8)**2) + m.x3/m.x9*exp(-0.5*((110 - m.x6)/m.x9)**2))) + 4*
                       log(0.398942448887604*(m.x1/m.x7*exp(-0.5*((115 - m.x4)/m.x7)**2) + m.x2/m.x8*exp(-0.5*((115 - 
