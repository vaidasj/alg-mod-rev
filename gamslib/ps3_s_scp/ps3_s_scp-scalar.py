...
m.c5 = Constraint(expr= - 0.91*m.x1 + m.x7 >= 0.1)

m.c6 = Constraint(expr= - 0.76*m.x2 + m.x8 >= 0.4)

m.c7 = Constraint(expr= - 0.91*m.x3 + m.x9 >= 0.9)

m.c8 = Constraint(expr= - 0.91*m.x1 + 0.91*m.x2 + m.x7 - m.x8 >= 0)

m.c9 = Constraint(expr= - 0.76*m.x2 + 0.76*m.x3 + m.x8 - m.x9 >= 0)

m.c10 = Constraint(expr= - 0.91*m.x3 + m.x9 >= 0)

m.c11 = Constraint(expr= - 0.91*m.x1 + m.x7 >= 0)

m.c12 = Constraint(expr=   0.76*m.x1 - 0.76*m.x2 - m.x7 + m.x8 >= 0)

m.c13 = Constraint(expr=   0.91*m.x2 - 0.91*m.x3 - m.x8 + m.x9 >= 0)
