...
m.c15 = Constraint(expr= - 12.7777777777778*m.x27 - 10.2222222222222*m.x28 + m.x44 - 0.255555555555556*m.x49
...
m.c18 = Constraint(expr=-0.0769230769230769*m.x37*m.x21 + m.x47 == 0)

m.c19 = Constraint(expr=-0.181818181818182*m.x38*m.x22 + m.x48 == 0)
...
m.c24 = Constraint(expr= - 9.44444444444444*m.x27 - 7.55555555555556*m.x28 + m.x42 - 0.188888888888889*m.x49
...
m.c26 = Constraint(expr=-(20*m.x27 + 16*m.x28 - 0.4*m.x42 - 0.4*m.x44 + 0.4*m.x49 + 0.4*m.x50)/m.x33 + m.x13 == 0)

m.c27 = Constraint(expr=-(30*m.x27 + 24*m.x28 - 0.6*m.x42 - 0.6*m.x44 + 0.6*m.x49 + 0.6*m.x50)/m.x34 + m.x14 == 0)

m.c28 = Constraint(expr=-m.x52*m.x37*m.x21 + m.x49 == 0)

m.c29 = Constraint(expr=-m.x53*m.x38*m.x22 + m.x50 == 0)
...
m.c35 = Constraint(expr=-1.78631298097427*(0.316984436431308*m.x21**0.5 + 0.683015563568692*m.x25**0.5)**2 + m.x23 == 0)

m.c36 = Constraint(expr=-1.8103795278422*(0.315975006847877*m.x22**0.5 + 0.684024993152123*m.x26**0.5)**2 + m.x24 == 0)

m.c37 = Constraint(expr=-(0.423659272868161*m.x33/((1.07692307692308 + m.x52)*m.x37))**2*m.x23 + m.x21 == 0)

m.c38 = Constraint(expr=-(0.425145461109024*m.x34/((1.18181818181818 + m.x53)*m.x38))**2*m.x24 + m.x22 == 0)

m.c39 = Constraint(expr=-(0.912870929175277*m.x33/m.x39)**2*m.x23 + m.x25 == 0)

m.c40 = Constraint(expr=-(0.920357986616844*m.x34/m.x40)**2*m.x24 + m.x26 == 0)

m.c41 = Constraint(expr=m.x52*(11.7 - m.x21) == 0)

m.c42 = Constraint(expr=m.x53*(1100 - m.x22) == 0)

m.c43 = Constraint(expr= - m.x21 >= -11.7)

m.c44 = Constraint(expr= - m.x22 >= -1100)

m.c45 = Constraint(expr=-2.42780549270868*(0.747349691412928*m.x19**1.5 + 0.252650308587072*m.x25**1.5)**
                        0.666666666666667 + m.x11 == 0)

m.c46 = Constraint(expr=-2.91102542459458*(0.809256430169454*m.x20**1.5 + 0.190743569830546*m.x26**1.5)**
                        0.666666666666667 + m.x12 == 0)

m.c47 = Constraint(expr=-(1.02120377146637*m.x31/m.x39)**(-2)*m.x11 + m.x25 == 0)

m.c48 = Constraint(expr=-(1*m.x32/m.x40)**(-2)*m.x12 + m.x26 == 0)

m.c49 = Constraint(expr=-(3.02076149339864*m.x31/m.x35)**(-2)*m.x11 + m.x19 == 0)

m.c50 = Constraint(expr=-(4.24264068711929*m.x32/m.x36)**(-2)*m.x12 + m.x20 == 0)

m.c51 = Constraint(expr= - m.x7 - m.x8 - m.x13 - m.x15 - m.x17 + m.x23 == 0)

m.c52 = Constraint(expr= - m.x9 - m.x10 - m.x14 - m.x16 - m.x18 + m.x24 == 0)

m.c53 = Constraint(expr= - m.x3 - m.x4 == -50)

m.c54 = Constraint(expr= - m.x5 - m.x6 == -40)
