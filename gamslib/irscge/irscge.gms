...
option NLP=convert;
option NLP=convert;
$ Title A CGE Model with Scale Economy in Ch. 10.6 (IRSCGE,SEQ=281)
...
F0(h,j) =(1-nu(j))*SAM(h,j)$(ord(h) eq 1)
        +SAM(h,j)$(ord(h) ne 1);
FC(j)   =nu(j)*SAM("CAP",j);
Y0(j)   =sum(h, F0(h,j));
X0(i,j) =SAM(i,j);
...
Model irscge /all/;
Solve irscge maximizing UU using nlp;
* ---------------------------------------------------------------------
...
dF(h,j)=(F.l(h,j)/F0(h,j)-1)*100$F0(h,j);
